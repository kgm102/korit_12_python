...
# 클래스 정의 예시
class WaffleMachine:
    pass            # 클래스나 함에 쓰면 오류가 안남

# 객체 생성 예시
waffle = WaffleMachine()
print(waffle)   # 결과값 : <__main__.WaffleMachine object at 0x000002139346B0E0>
'''
클래스의 구성
...
1. 클래스의 기본 구성
    ...
    self 키워드 : 인스턴스 변수에서 각각의 객체를 의미하기 위해서 self 키워드를 붙여줍니다. 인스턴스 메서드에서의 첫 번째 매개변수로 '항상' self를 추가해야합니다(그래서 자종 생성됩니다).
