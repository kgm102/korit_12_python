'''
1. 클래스 변수 vs. 인스턴스 변수
    인수턴스 변수 : 인스턴스 마다 서로 다른 값
    클래스 변수 : 모든 인스턴ㅅ가 동일한 값을 지니는 변수
...
'''
객체명.클래스변수명을 통해서 클래스 변수에 접근이 가능하긴 한데, 클래스 내부코드를 보기 전까지는 country라는 변수가 인스턴스 변수인지 클래스 변수인지 알 방법이 없습니다.

이상을 이유로 클래스 변수를 확인하조가 할 때는 객체명.클래스변수명 보다는 클래명.클래스변수명을 통해서 참조하는 것이 구너장됩니다.
...
#객체  생성
person2 = Korean2()
person2.trip('일본') # 객체명.클레스메서드() 호출도 가능하긴 합니다. 근데 마찬가지로 권장되지 않습니다.
...
3. 정적 메서드(static method)
    ...
    인스턴스를 생성하지 않아도 사용할 수 있음 - 클래스 메서드와으이 공통점 # 2
...
class Bag:
    ...
    def sell(cls):
        print('가방이 팔렸습니다.')
        cls.count -= 1 # 얘는 클래스 메서드니까 Bag.count가 아니라 cls.count 입니다.
